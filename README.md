...
✅ [Bundling](#bundling) with [Rollup](https://rollupjs.org/) (pure ESM).<br>
...
1. The `index.ts` file in each named CLI subdirectory will be picked up by the [bundler](#bundling) and compiled into a CLI command. So `src/cli/mycli/index.ts` will be compiled into `dist/cli/mycli/index.js`.
...
This template uses [Rollup](https://rollupjs.org) to bundle your code into pure ESM output. See the [References](#references) section for more detailed notes. It creates:

- **ESM library output**, for import into TS/JS code.
...
See [`rollup.config.ts`](https://github.com/karmaniverous/npm-package-template-ts/tree/main/rollup.config.ts) for details.
...
[`rollup.config.ts`](https://github.com/karmaniverous/npm-package-template-ts/tree/main/rollup.config.ts) is configured to treat all non-dev and peer dependencies listed in `package.json` as external dependencies. This means these dependencies will not be bundled into your package, but should be installed as a matter of course when you run `npm install`.

It is possible for external dependencies to creep into your build. For example, let's say you have a dev dependency that you referenced in runtime code, but is actually included as a runtime dependency by some other package. In this case the dependency would _not_ be listed as an external dependency by `rollup.config.ts`, which could shift your distribution file locations and break your package.
...
If you've been careful to document your code according to [TSDoc](https://tsdoc.org/) conventions, then when you run `npm run docs`, [TypeDoc](https://typedoc.org/) will generate API documentation, wrap it up with your README file, and publish it in browsable form.

The docs are deployed automatically to GitHub Pages via the included [docs workflow](.github/workflows/docs.yml) on every push to `main`.
...
Note that this README file contains a link in the header to this documentation site that does NOT appear in the copy of the README on the documentation site's home page.
...
Anything between these tags (as well as the tags themselves) will be excluded from the documentation site. Note that you can also use these tags to exclude content from your inline Typescript documentation!
